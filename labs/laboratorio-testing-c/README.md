...
> R: No coincide. Antes de corregir el bug, el programa mostraba $2050 (sumaba solo el precio unitario de cada producto una vez, sin multiplicar por la cantidad).
...
> R: Inicializa el campo `cantidad` en 0. Es importante llamarla antes de usar el carrito porque `Carrito c;` se declara en la pila sin inicializar: su memoria tiene basura (valores indeterminados), y si no se pone `cantidad` en 0 explícitamente, funciones como `carrito_agregar` o `carrito_total` leerían y escribirían en posiciones incorrectas del array `items`.
...
> R: Pasa. Con cantidad = 1, multiplicar por la cantidad o no da el mismo resultado (350 * 1 = 350), así que este caso no alcanza a exponer el bug de `carrito_total`.
...
> R: Falla. Esperaba 700 (350 x 2) y obtuvo 350, porque `carrito_total` no multiplica el precio por la cantidad.
...
> R: En `carrito_total` (src/carrito.c), en la línea del `for`: `total += c->items[i].precio;` — suma solo el precio unitario, ignorando la cantidad.
...
> R: Antes de corregir el bug, el test falla: el 5to `carrito_agregar` devuelve 1 (éxito) en lugar de 0, y además escribe fuera de los límites del array `items` (buffer overflow), porque `MAX_ITEMS = 4` y los índices válidos son 0-3.
...
> R: La condición era `c->cantidad <= MAX_ITEMS`, que debería ser `c->cantidad < MAX_ITEMS` (con `<=` permite escribir en `items[4]`, que no existe ya que los índices van de 0 a 3).
...
> R: Sí. Las líneas de `carrito_descuento` (el `return total - (total * porcentaje / 100);` y la firma de la función) aparecen con `#####`, porque `tests/test_unitarios.c` nunca llama a `carrito_descuento` — esa función solo se ejercita desde `tests/test_integracion.c` y `src/programa.c`, y `make cobertura` mide cobertura únicamente con `test_unitarios`.
...
> R: Un test unitario verifica una sola función de forma aislada; un test de integración verifica que varias funciones funcionan bien combinadas (por ejemplo, `carrito_agregar` + `carrito_total` + `carrito_descuento` en una secuencia de compra). El bug de `carrito_total` lo detectó primero un test unitario (`test_total_con_cantidad`, en la Parte 5), antes de llegar a los tests de integración.
...
> R: En C los arrays no tienen chequeo de límites (bounds checking): escribir en `items[4]` cuando el array tiene 4 elementos (índices 0-3) simplemente escribe en la memoria contigua, que puede pertenecer a otra variable, corrompiendo datos de forma silenciosa o causando comportamiento indefinido (crashes intermitentes, vulnerabilidades de seguridad explotables). Python y Java, en cambio, chequean los índices en cada acceso y lanzan una excepción (`IndexError`, `ArrayIndexOutOfBoundsException`) de inmediato, evitando que el error pase desapercibido.
...
> R: Los tests verifican el comportamiento real ejecutando el código con casos concretos, en vez de confiar en la lectura manual, que es propensa a errores y no escala a medida que el código crece. Además, una vez escritos, los tests quedan como una red de seguridad reutilizable (regresión): cualquier cambio futuro que rompa algo se detecta automáticamente, algo que una revisión visual del código no garantiza.
...
> R: Porque con cantidad = 1, multiplicar por la cantidad o no da el mismo resultado (precio * 1 = precio), así que el valor buggeado (sumar solo el precio) coincide por casualidad con el valor correcto. Es un caso límite que "esconde" el bug; recién con cantidad = 2 el resultado buggeado (350) difiere del correcto (700) y el test lo expone.
